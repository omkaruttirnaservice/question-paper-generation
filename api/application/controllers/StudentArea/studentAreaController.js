...
const StudentAreaController = {
    ...
    saveFormFillingIP: asyncHandler(async (req, res) => {
        ...
        if (!form_filling_server_ip) throw new ApiError(404, 'Invalid IP address form filling.');
        if (!exam_panel_server_ip) throw new ApiError(404, 'Invalid IP address exam server.');

        await studentAreaModel.addFormFillingIP({
            form_filling_server_ip,
            exam_panel_server_ip,
        });
        return res.status(201).json(new ApiResponse(201, null, 'Successfully added new ip.'));
    }),
    ...
    deleteFormFillingIP: asyncHandler(async (req, res) => {
        ...
        return res.status(200).json(new ApiResponse(200, null, 'Successfully deleted IP.'));
    }),
    ...
    downloadCentersList: asyncHandler(async (req, res) => {
        ...
        const [error, data] = await StudentAreaController.downloadCenterListFromFormFilling(ip);
        console.log(error, '==error==');
        if (error) {
            throw new ApiError(400, error);
        }
    ...
    downloadCenterListFromFormFilling: async (ip) => {
        try {
            let _centersListRes = await fetch(`${ip}/api/student-data/download-centers-list`);
            ...
            if (error.type == 'fetch failed') {
                ...
                return [
                    error?.message || 'Form filling server error while downloading centers list.',
                ];
            }
        }
    },
    ...
    downloadStudentQuestionPaper: asyncHandler(async (req, res) => {
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, null, 'Successfully downlaoded student question paper.'));
    }),
    ...
    async getQuestionPaperFromExamServer(exam_panel_server_ip) {
        ...
        try {
            ...
            const _studentQuestionPaperResponseJson = await _studentQuestionPaperResponse.json();
