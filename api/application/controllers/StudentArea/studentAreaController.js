...
const StudentAreaController = {
  ...
  getStudentSearchPageFilters: asyncHandler(async (req, res) => {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, _filters, "Students list page filters"));
  }),
  ...
  downloadCentersList: asyncHandler(async (req, res) => {
    ...
    let [_saveCenterListRes] =
      await studentAreaModel.saveCentersList(centersListToSave);
  ...
  uploadPresentStudentsListToFormFilling: asyncHandler(
    async function (req, res) {
      ...
      // 3. Send data to form filling server
      const [error, result] = await StudentAreaController.markPresentStudents(
        data,
        _sl,
      );
